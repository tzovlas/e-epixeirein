...
class OneStore_Compatibility_WooCommerce {
	...
	private $filter_sidebar_content = false;
	...
	protected function __construct() {
		...
		require_once get_template_directory() . '/inc/compatibilities/woocommerce/class-single-product.php';

		add_action( 'onestore/customize/setup', [ __CLASS__, 'load_customize_settings' ] );
		...
		add_filter( 'woocommerce_widget_cart_is_hidden', '__return_false', 199 );
		add_filter( 'woocommerce_add_to_cart_redirect', array( $this, 'cart_buy_now_redirect_url' ), 30, 2 );

	}

	public static function load_customize_settings() {
		require_once ONESTORE_INCLUDES_DIR . '/compatibilities/woocommerce/customizer/settings/wc-product-single.php';
	}
	...
	public function modify_template_hooks_after_init() {
		...
		if ( is_product() ) {
			...
			remove_action('woocommerce_single_product_summary', 'woocommerce_before_single_product_summary', 10 );
		...
		if ( onestore_get_theme_mod( 'woocommerce_single_buy_now' ) ) {
			add_action( 'woocommerce_after_add_to_cart_button', array( $this, 'buy_now_button' ) );
		}

	}

	public function cart_buy_now_redirect_url( $url, $adding_to_cart ) {
		if ( isset( $_REQUEST['onestore_buy_now'] ) && 1 == absint( $_REQUEST['onestore_buy_now'] ) ) {
	...
	public function buy_now_button() {
		global $product;
		$classes = 'button buy-now-btn wc-buy-now-btn';
		if ( 'variable' == $product->get_type() ) {
			$classes .= ' single_add_to_cart_button';
		}
		?>
		<button type="submit" name="onestore_buy_now" value="1" class="<?php echo $classes; // WPCS: XSS ok. ?>"><?php esc_html_e( 'Buy Now', 'onestore' ); ?></button>
		<?php
	}

	public function widget_filter_li( $term_html, $term, $link, $count ) {
		$term_html = '<div class="v-item-inner">' . $term_html . '</div>'; // WPCS: XSS ok.
		return $term_html;
	}
	...
	public function modify_template_hooks() {
		...
		add_filter( 'yith_wcwl_add_to_wishlist_params', array( $this, 'yith_wcwl_add_to_wishlist_params' ) );

	}
	public function yith_wcwl_add_to_wishlist_params( $additional_params ) {
		if ( ! isset( $additional_params['link_classes'] ) ) {
			$additional_params['link_classes'] = '';
		}
		$additional_params['in_block_item'] = '';
	...
	public function filter_button() {
		...
		<span class="filter-btn">
			<button data-target="product-filter-modal" class="action-toggle button has-txt popup-toggle"><?php onestore_icon( 'filter' ); ?> <?php _e( 'Filter', 'onestore' ); ?></button>
		</span>
		<?php
	}
	...
	public function add_single_wishlist_button() {
		...
		if ( $show ) {
			echo do_shortcode( '[yith_wcwl_add_to_wishlist]' ); // WPCS: XSS ok.
		}
	}
