...
class OneStore_Admin {
	...
	protected function __construct() {
		...
		if ( is_admin() ) {
			...
			if ( isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) {
				wp_redirect( 'themes.php?page=onestore' );
			}
		}
	...
	public function add_rating_notice() {
		...
		$dismissed = get_transient( 'onestore_rating_notice_is_dismissed' );
		...
		<div class="notice notice-info onestore-rating-notice">
			...
			<p>
				...
				<a href="#" class="onestore-rating-notice-close button-link" data-onestore-rating-notice-repeat="2"><?php esc_html_e( 'Nope, maybe later', 'onestore' ); ?></a>&nbsp;&nbsp;&nbsp;
	...
	public function reset_rating_notice_flag() {
		delete_transient( 'onestore_rating_notice_is_dismissed' );
	}
	...
	public function ajax_dismiss_rating_notice() {
		...
		if ( -1 == $repeat_after ) {
			...
			set_transient( 'onestore_rating_notice_is_dismissed', 1, 90 * DAY_IN_SECONDS );
		} else {
			// Repeat rating notice later.
			set_transient( 'onestore_rating_notice_is_dismissed', 1, 7 * DAY_IN_SECONDS );
		}
	...
	public function ajax_install_sites_import_plugin() {
		...
		$path = 'onestore-sites/onestore-sites.php';
	...
	public function render_admin_page() {
		?>
		<div class="wrap onestore-admin-wrap  fs-section fs-full-size-wrapper <?php echo esc_attr( onestore_is_plus() ? 'onestore-pro-installed ' : '' ); ?>">
			...
			<h2 class="nav-tab-wrapper">
				<a href="<?php echo esc_url( admin_url( '/themes.php?page=onestore' ) ); ?>" class="nav-tab fs-tab nav-tab-active home"><?php _e( 'Overview', 'onestore' ); ?></a>
			  </h2>
	...
	private function renter_module_list() {
		$all_modules = array();
	...
	public function render_admin_page__modules() {
		...
		if ( ! $tab ) {
			$this->renter_module_list();
		} else {
			do_action( 'onestore/admin/dashboard/setting_tab', $tab );
		}
	}
	...
	public function render_sidebar__links() {
		...
		if ( ! onestore_is_plus() ) {
			?>
			<div class="onestore-admin-noti postbox">
			<h2 class="hndle"><?php esc_html_e( 'Donate', 'onestore' ); ?></h2>
			<div class="inside">
				<p>
					<?php esc_html_e( 'If you like OneStore theme, buy us a beer by upgrading to OneStore Plus version. Thank you!', 'onestore' ); ?>
				</p>
				<a class="button button-primary" href="<?php echo esc_url( ONESTORE_PLUS_URL ); ?>" target="_blank"><?php esc_html_e( 'Upgrade Now', 'onestore' ); ?></a>
			</div>
		</div>
			<?php
		}
